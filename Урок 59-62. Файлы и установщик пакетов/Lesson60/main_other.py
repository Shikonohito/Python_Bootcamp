...
class DB:
    def __init__(self, student_list=None, group_list=None):
        if student_list:
...
#
# data_base = DB(student_list, group_list)
...
def save_data():
    data_base.save_db("database02.dat")


def load_data():
    global data_base
    data_base = DB.load_db("database02.dat")

    # Обновляем виджеты для правильного отображения данных
    fs_fill_student_listbox()
    fg_fill_group_listbox()
    fgd_student_combobox["values"] = data_base.student_list
    fgd_group_combobox["values"] = data_base.group_list
...
def fs_add_student():
    ...
    data_base.student_list.append(new_student)
...
def fs_delete_student():
    ...
    if len(student_listbox_ind) > 0:
        ...
        data_base.student_list.pop(selected_student_index)
...
def fs_show_student_data():
    ...
    if len(student_listbox_ind) > 0:
        ...
        student = data_base.student_list[selected_student_index]
...
def fg_fill_group_listbox():
    ...
    for group in data_base.group_list:
...
def fg_add_group():
    ...
    # Добавляем объект в бэкенд
    data_base.group_list.append(new_group)
...
def fg_delete_group():
    ...
    if len(group_listbox_ind) > 0:
        ...
        # Пытаемся удалить из бэкенда
        data_base.group_list.pop(selected_group_index)
...
def fg_show_group_data():
    ...
    if len(group_listbox_ind) > 0:
        ...
        # Запрашиваем из бэкенда объект
        group = data_base.group_list[selected_group_index]
...
fgd_group_combobox = ttk.Combobox(frame_group_details, values=data_base.group_list, font=("Arial", 18), state="readonly")
...
fgd_student_combobox = ttk.Combobox(frame_group_details, values=data_base.student_list, font=("Arial", 18), state="readonly")
...
def fgd_select_group():
    ...
    selected_group = data_base.group_list[group_index]
...
def fgd_add_student_to_group():
    ...
    selected_student = data_base.student_list[student_index]
