# Дополните код, указанный ниже, ещё двумя полями - фамилией и возрастом.
# Также измените код изменения клиента в бэкенде,
# а во фронтенде добавление, изменение, показ данных клиента.

customer_1 = {"id": "ABC1234", "name": "Tom", "l_name": "Jackson", "age": 25}
customer_2 = {"id": "DEF5869", "name": "Bob", "l_name": "Morgan", "age": 28}
customer_3 = {"id": "QWE1556", "name": "Kate", "l_name": "Lebra", "age": 27}
customer_list = [customer_1, customer_2, customer_3]


def backend_get_customer_index(customer_id: str) -> int:
    found_index = -1
...
def backend_get_customer(customer_id: str) -> dict | None:
    index = backend_get_customer_index(customer_id)
    if index != -1:
...
def backend_add_customer(new_customer: dict) -> bool:
    is_success = False
    index = backend_get_customer_index(new_customer["id"])
    if index == -1:
...
def backend_change_customer(customer_to_change_id: str, changed_customer: dict) -> bool:
    is_success = False
    ...
    if customer_to_change:
        ...
        changed_customer_index = backend_get_customer_index(changed_customer_id)
        if customer_to_change_id == changed_customer_id or changed_customer_index == -1:
            ...
            customer_to_change["l_name"] = changed_customer["l_name"]
            customer_to_change["age"] = changed_customer["age"]
            is_success = True
    return is_success


def backend_remove_customer(customer_id: str) -> bool:
    is_success = False
    index = backend_get_customer_index(customer_id)
    if index != -1:
...
customer_l_name_lbl = tkinter.Label(customer_frame, text="Last name:", font=("Arial", 18))
customer_l_name_lbl.place(x=340, y=180)

customer_l_name_entry = tkinter.Entry(customer_frame, font=("Arial", 18))
customer_l_name_entry.place(x=340, y=220)

customer_age_lbl = tkinter.Label(customer_frame, text="Age:", font=("Arial", 18))
customer_age_lbl.place(x=340, y=260)

customer_age_entry = tkinter.Entry(customer_frame, font=("Arial", 18))
customer_age_entry.place(x=340, y=300)
...
def add_customer():
    ...
    new_l_name = customer_l_name_entry.get()
    new_age = int(customer_age_entry.get())
