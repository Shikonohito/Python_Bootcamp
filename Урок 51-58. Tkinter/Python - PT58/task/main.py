...
# ДОПОЛНИТЕ ФУНКЦИЮ ЧТЕНИЕМ ПОЛА И ОБНОВЛЕНИЕМ КОМБОБОКСА СТУДЕНТА
def fs_add_student():
    ...
    new_student = Student(new_id, new_name, new_age, "GENDER")
...
# ДОПОЛНИТЕ ФУНКЦИЮ ОБНОВЛЕНИЕМ КОМБОБОКСА СТУДЕНТА
...
# ДОПОЛНИТЕ ФУНКЦИЮ ОБНОВЛЕНИЕМ КОМБОБОКСА ГРУППЫ
def fg_add_group():
    # Считываем данные из полей фронтенда
...
def fg_show_group_data():
    ...
    if len(group_listbox_ind) > 0:
        ...
        if group:
            group_data = f"Name: {group.get_name()}\nTeacher name: {group.get_teachers_name()}"
...
# ОПИШИТЕ ФУНКЦИЮ ДОБАВЛЕНИЯ С ИСПОЛЬЗОВАНИЕМ КОМБОБОКСОВ
